                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <p className="font-bold text-gray-800">{pedido.customer_name}</p>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${estado.color}`}>
                        {estado.label}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">📍 {pedido.customer_address}</p>
                    <p className="text-sm text-gray-500">📞 {pedido.customer_phone}</p>
                    {pedido.notes && (
                      <p className="text-sm text-gray-500 mt-1">📝 {pedido.notes}</p>
                    )}
                    <p className="text-xs text-gray-400 mt-2">
                      #{pedido.tracking_code} · {new Date(pedido.created_at).toLocaleTimeString()}
                    </p>
                  </div>
